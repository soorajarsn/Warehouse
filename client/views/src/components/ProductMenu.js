...
function SlidingFromLeftAccordian(props) {
  function showPanel(event) {
    let id;
    if (!event.target.getAttribute("area-controls")) id = event.target.parentNode.getAttribute("area-controls");
    else id = event.target.getAttribute("area-controls");
    document.querySelector(`#${id}`).classList.toggle("is-open");
  }
  return (
    <div
      className="sliding-left-accordian medium-margin full-width flex justify-space-between link"
      area-controls={props.device + "_" + props.id}
      onClick={event => showPanel(event)}>
      {props.title} <i className="fas fa-angle-right"></i>
    </div>
  );
}
function SocialMediaCard(props) {
  return (
    <div className="medium-margin flex link">
...
function HandleClickOnAccordian(event) {
  ...
  if (!event.target.classList.contains("accordian")) {
    ...
    } else panel = parent.nextElementSibling;
  } else panel = event.target.nextElementSibling;
  ...
  return (
    <div className="accordian-main medium-padding medium-bold-font" area-expanded="false">
      <div className="accordian flex justify-space-between link full-width" onClick={event => HandleClickOnAccordian(event)}>
        <div className="icon-and-content">{props.title}</div>
      ...
      <ul className="panel">
        {props.subCategories.map((subCategory, index) => (
          <li key={index} className="small-margin xsmall-font regular-font">
            <a href={"/products?productClass=" + props.productClassQuery + "&subCategory=" + subCategory}>{subCategory}</a>
          </li>
        ))}
      </ul>
    </div>
  ...
  return (
    <div className="productClass-panel-container">
      ...
      <div className="accordian-container medium-margin-left medium-margin-right">
        {props.productClass.products.map((product, index) => (
          <Accordian key={index} productClassQuery={props.productClass.productClassQuery} title={product.category} subCategories={product.subCategories} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="product-bar-container flex flex-column medium-margin-left medium-margin-right regular-font">
        {props.productClasses.map((item, index) => (
          <SlidingFromLeftAccordian key={item.id} id={item.id} title={item.productClass} device={"mobile"} />
        ))}
      </div>
    </div>
  );
    ...
    <div className="menu-container position-fixed" area-hidden="true">
      <div className="product-bar-container-main position-fixed full-width">
        ...
        <div className="product-bar-container flex large-padding flex-column medium-margin-left medium-margin-right regular-font">
          {menuItems.map(productClass => (
            <SlidingFromLeftAccordian key={productClass.id} id={productClass.id} title={productClass.name} device="mobile" />
          ))}
        </div>
        <hr />
        <div className="need-help-container large-padding limit-width flex flex-column medium-margin-left">
          ...
          <div className="medium-margin flex">
            <MobileSvg />
            Call us 1888-9292-3333
          </div>
...
function DesktopMainMenu(props) {
  function showPanel(event) {
    let id;
    let menuItem;
    if (!event.target.getAttribute("area-controls")) menuItem = event.target.parentNode;
    else menuItem = event.target;
    ...
    if (isOpen) menuItem.classList.add("active");
    else menuItem.classList.remove("active");
  }
  return (
    <div
      className="sliding-left-accordian medium-margin medium-margin-left medium-margin-right full-width flex justify-space-between link"
      area-controls={"desktop_" + props.id}
      onClick={event => showPanel(event)}>
      {props.title} <i className="fas fa-angle-right"></i>
    </div>
  );
...
function ExpandedIndividualProductMenu(props) {
  return (
    <div className="expanded-menu flex justify-space-between medium-margin-left medium-margin-right large-padding" id={"desktop_" + props.productClass.id}>
      {props.giveTitle && (
        <a href={`/products?productClass=${props.productClass.productClassQuery}`}>
          {" "}
          <h2 className="medium-font small-margin title medium-bold-font">{props.productClass.productClass}</h2>{" "}
        </a>
      )}
      {props.productClass.products.map(item => (
        <div key={item.category} className="flex flex-column menu-items">
          <a href={`/products?productClass=${props.productClass.productClassQuery}&category=${item.category}`}>
            {" "}
            <h3 className="medium-bold-font small-font small-margin">{item.category}</h3>{" "}
          </a>
          <ul className="sub-categories">
            {item.subCategories.map(subCategory => (
              <a key={subCategory} href={`/products?productClass=${props.productClass.productClassQuery}&subCategory=${subCategory}`}>
                <li className="color-black small-margin xxsmall-font">{subCategory}</li>{" "}
              </a>
            ))}
          </ul>
        </div>
  ...
  return (
    <div className="desktop-menu-container-main hidden full-width flex position-relative flex-column ">
      <div className="desktop-menu-container flex full-width limit-width justify-space-between xsmall-padding xsmall-font">
        {menuItems.map(productClass => (
          <DesktopMainMenu key={productClass.id} id={productClass.id} title={productClass.name} device="desktop" />
        ))}
      </div>
