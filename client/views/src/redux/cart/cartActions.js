...
export const addToCart = (id,size) => {
    return (dispatch,getState) => {
        ...
        Axios.post('/post/addCart',{id,size},config).then(response => {
...
export const removeFromCart = (id) => {
    return (dispatch,getState) => {
        ...
        .catch(err => {
            if(err.response)
                dispatch(removeFromCartError(err.response.data.errorMsg));
            else
                dispatch(removeFromCartError("Something went wrong"));
        });
    }
}